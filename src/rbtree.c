 ...
 * described in Introduction to Algorithm Third Edition.
 */
...
rbtree_node_t* rbtree_node_new(rbtree_key_t key, rbtree_val_t val) {
...
rbtree_node_t* rbtree_lowbound(rbtree_node_t* root, rbtree_key_t key) {
  if (root == rbtree_nil)
    return rbtree_nil;
  rbtree_node_t* lb;
  if ((lb = rbtree_lowbound(root->left, key)) != rbtree_nil)
    return lb;
  if (root->key >= key)
    return root;
  return rbtree_lowbound(root->right, key);
}
