...
static inline rbtree_node_t* rbtree_insert_key(rbtree_node_t* root,
                                               rbtree_key_t key,
                                               rbtree_val_t val) {
  return rbtree_insert(root, rbtree_node_new(key, val));
}

static inline rbtree_node_t* rbtree_delete_key(rbtree_node_t* root,
                                               rbtree_key_t key) {
...
static void test(void) {
  ...
  for (int i = 0; i < sizeof(keys) / sizeof(rbtree_key_t); ++i) {
    root = rbtree_insert_key(root, keys[i], NULL);
    verify(root);
  }
